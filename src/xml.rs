...
pub(crate) fn parse_propfind_response(s: &str) -> anyhow::Result<DirectoryListing<String>> {
    todo!()
}
...
mod tests {
    ...
    #[test]
    fn test01() {
        let s = include_str!("testdata/response.xml");
        let dl = parse_propfind_response(s).unwrap();
